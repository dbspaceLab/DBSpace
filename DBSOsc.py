...
def Extract_Osc_Feats(rawdata,start_t=0,end_t=1,chann_idxs=[0,2],Fs=422):
    ...
    OscState['GC_R'] = (OscState['HarmPow']*OscState['StimPow'])/(OscState['HarmPow'] + OscState['StimPow'])
...
def Phase_to_Matrix(ph_list,pt_list,dataF,mean_subtr=False,del_nan = False):
    ...
    for phh, phase in enumerate(ph_list):
        for pp, patient in enumerate(pt_list):
            ...
            #THIS STEP SUCKS
            #check if we have 'GoldPSD' or 'MeanPSD' as a key here
            if 'MeanPSD' in dataF[patient][phase]:
                meankey = 'MeanPSD'
            else:
                meankey = 'GoldPSD'
            
            if len(dataF[patient][phase][meankey]['LOGPxx']) > 0:
            #    EPHYS_out[:,:,phh,pp] = dataF[patient][phase]['Recordings'][0]['PSD']['Y']
                ...
                #append the Gold PSD
                X.append(dataF[patient][phase][meankey]['LOGPxx'])
                ...
                if not del_nan:
                    X.append(np.nan * np.zeros_like(dataF['DBS901']['C01'][meankey]['LOGPxx']))
                    Y.append(dataF[patient][phase]['HDRS17'])
                else:
                    pass
...
def raw_pow(Pxx,F,band):
    #check if the dimensions of Pxx are as expected: CHANN x NFFT
