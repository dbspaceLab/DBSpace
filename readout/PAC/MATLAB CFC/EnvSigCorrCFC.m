function MIs = EnvSigCorrCFC(oscAmpMod,oscForPhase,freqForAmp,freqForPhase)
%ENVSIGCORRCFC Calculates and displays the CFC Comulolograms based on inputs
%   USAGE: MIs = EnvSigCorrCFC(oscAmpMod,oscForPhase,freqForAmp,freqForPhase)
...
%   around freqForPhase with some small bandwidth   
    ...
    for cc = 1:length(freqForAmp)
        for rr = 1:length(freqForPhase)
            ...
            phaseOsc = real(oscForPhase{1,rr});
